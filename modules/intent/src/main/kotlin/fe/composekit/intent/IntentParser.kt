...
public open class IntentParser internal constructor(
    ...
    public fun parseSearchIntent(intent: SafeIntent): String? {
    ...
    public fun getUriFromIntent(intent: SafeIntent): IResult<Uri> {
    ...
    private fun tryParseWebUriStrict(text: CharSequence, allowInsecure: Boolean = true): IResult<Uri> {
        ...
        if (!isWebStrict(str, allowInsecure)) return +NoUriFoundRecoverableException(str)
    ...
    private fun isWebStrict(url: String, allowInsecure: Boolean = true): Boolean {
...
public sealed class UriException : Exception()

public data object NoUriFoundException : UriException() {
    private fun readResolve(): Any = NoUriFoundException
}

public data object UriParseException : UriException() {
    private fun readResolve(): Any = UriParseException
}

public data class NoUriFoundRecoverableException(val recoverable: String, val wrappedEx: Throwable? = null) : UriException() {

}

public data class NoSuchExtraException(val name: String) : UriException()

public data class UnsupportedIntentActionException(val action: String?) : UriException()
